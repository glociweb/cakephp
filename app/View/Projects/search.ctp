...
?>
<?php
echo $this->Form->create('Search', array('url' => array('controller' => 'projects', 'action' => 'search', 'admin' => false), 'class' => ''));
echo $this->Form->input('query', array('plasceholder' => __('Search...'), 'label' => __('Search')));
echo $this->Form->end();
?>
...
<?php if (!empty($results['Phase'])): ?>
    ...
    <?php if (count($results['Phase']) == 15) echo $this->Html->link(__('Search all Phases'), array('controller' => 'projects', 'action' => 'search_phases', $query), array('class' => 'btn btn-mini')); ?>
<?php endif; ?>
...
<?php if (!empty($results['Comment'])): ?>
    ...
    <?php if (count($results['Comment']) == 15) echo $this->Html->link(__('Search all Comments'), array('controller' => 'projects', 'action' => 'search_comments', $query), array('class' => 'btn btn-mini')); ?>
<?php endif; ?>


<?php if (empty($results['Project']) && empty($results['Phase']) && empty($results['Comment'])): ?>
