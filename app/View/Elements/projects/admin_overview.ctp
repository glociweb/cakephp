    ...
    <div class="row-fluid">
        <div class="span6">
            <div class="pull-right">
                ...
                ?>
                <?php echo $this->Layout->editActions(array('view', 'edit', 'delete', 'addphase'), array('controller' => 'projects', 'id' => $project['id'], 'btn' => '')); ?>
            </div>
            <h2><?php echo $this->Html->link($project['title'], array('action' => 'view', $project['id'])); ?></h2>
            <?php echo $project['archived'] ? '<span class="label label-info"><i class="ico-database_key" data-rel="tooltip" data-original-title="' . __('archived') . '"></i> ' . __('archived') . '</span>' : ''; ?>
            <p>
                <?php echo $this->Text->truncate($project['description'], 150, array('exact' => false, 'ending' => ' [...]')); ?>
                <?php
                $lineBreak = trim($project['description'] == '') ? '' : '<br/>';
                $clientNames = Set::extract('{n}.title', $project['Client']);
                if ($clientNames != null)
                    echo $lineBreak . '<strong>' . __('Clients:') . '</strong> ' . $this->Text->toList($clientNames, __('and'));
                else
                    echo $lineBreak . '<strong>' . __('Clients:') . '</strong> ' . __('none');
                ?><br/>
