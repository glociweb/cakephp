...
(function (tinymce, $) {
    ...
    AutoComplete.prototype = {
		...
		processdepartment: function(a) {
			alert(a);
            if (this.hasFocus) {
                var c = this,
                    d = [],
                    e = b.grep(a, function(a) {
                        return c.matcher(a)
                    });
                e = c.sorter(e), e = e.slice(0, this.options.items), b.each(e, function(a, f) {
                    var g = b(c.render(f));
                    g.html(g.html().replace(g.text(), c.highlighter(g.text()))), b.each(e[a], function(a, b) {
                        g.attr("data-" + a, b)
                    }), d.push(g[0].outerHTML)
                }), d.length ? this.$dropdown.html(d.join("")).show() : this.$dropdown.hide()
            }
        },
